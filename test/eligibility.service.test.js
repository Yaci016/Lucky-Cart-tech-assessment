...
describe("Eligibility", () => {
  ...
  describe("And condition", () => {
    ...
    it("should not be eligible when and condition doesn't have minimum ammount of conditions", () => {
      const cart = { total: 20 };
      const criteria = { total: { and: { gt: 10 } } };
      const eligibilityService = new EligibilityService();
      const actualEligibility = eligibilityService.isEligible(cart, criteria);
      should(actualEligibility).be.false();
    });
  ...
  describe("Or condition", () => {
    it("should not be eligible when or condition doesn't have minimum ammount of conditions", () => {
      const cart = { total: 20 };
      const criteria = { total: { or: { gt: 10 } } };
      const eligibilityService = new EligibilityService();
      const actualEligibility = eligibilityService.isEligible(cart, criteria);
      should(actualEligibility).be.false();
    });
